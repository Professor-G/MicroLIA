...
class Classifier:
    ...
    Args:
        positive_class (ndarray, optional): 2D array of size (n x m), where n is the
        ...
        negative_class (ndarray, optional): 1D array containing the corresponing labels. Defaults to None,
            in which case a model can be loaded using the class attributes..
    ...
    def __init__(self, positive_class=None, negative_class=None, val_positive=None, val_negative=None, img_num_channels=1, 
        ...
        balance_val=True, opt_max_min_pix=None, opt_max_max_pix=None, metric='loss', average=True, test_positive=None, 
        ...
        if self.epochs != 0:
            ...
            if self.opt_aug:
                ...
                augmented_images = augmentation(channel1=channel1, channel2=channel2, channel3=channel3, batch=self.best_params['batch'], 
                    width_shift=self.shift, height_shift=self.shift, horizontal=True, vertical=True, rotation=self.best_params['rotation'], 
                ...
                if self.balance_val:
                    class_2 = self.negative_class[:len(class_1)]   
                else:
            ...
            if self.opt_model:
                ...
                if self.limit_search is False:
                    self.model, self.history = AlexNet(class_1, class_2, img_num_channels=self.img_num_channels, 
                        ...
                        regularizer=self.best_params['regularizer'], activation_conv=self.best_params['activation_conv'], activation_dense=self.best_params['activation_dense'], 
                        pooling_1=self.best_params['pooling_1'], pooling_2=self.best_params['pooling_2'], pooling_3=self.best_params['pooling_3'],  
                        conv_init=self.best_params['conv_init'], dense_init=self.best_params['dense_init'], verbose=self.verbose)
                else:
                    self.model, self.history = AlexNet(class_1, class_2, img_num_channels=self.img_num_channels, 
                        normalize=self.normalize, min_pixel=min_pix, max_pixel=max_pix, val_positive=val_class_1, val_negative=val_class_2, 
                        epochs=self.train_epochs, batch_size=self.best_params['batch_size'], optimizer=self.best_params['optimizer'],
                        lr=self.best_params['lr'], momentum=momentum, decay=decay, nesterov=nesterov, 
...
def AlexNet(positive_class, negative_class, img_num_channels=1, normalize=True, 
        ...
        activation_conv='relu', activation_dense='relu', regularizer='local_response', padding='same', 
        ...
        dense_neurons_2=4096, dropout_1=0.5, dropout_2=0.5, optimizer='sgd', early_stop_callback=None, checkpoint=False, verbose=1):
        """
        ...
        Args:
            ...
            early_stop_callback (list, optional): Callbacks for early stopping and pruning with Optuna, defaults
                to None. Should only be used during optimization, refer to MicroLIA.optimization.objective_cnn().
            ...
            regularizer (str): The regularizer to use, can be None.
        Returns:
            The trained CNN model and accompanying history.
        """
        
        if len(positive_class.shape) != len(negative_class.shape):
            raise ValueError("Shape of positive and negative class data must be the same.")
        ...
        if verbose == 1:
            print_params(batch_size, lr, decay, momentum, nesterov, loss, optimizer, regularizer, conv_init, activation_conv, dense_init, activation_dense,
        ...
        model.add(Conv2D(filter_1, filter_size_1, strides=strides_1, activation=activation_conv, input_shape=input_shape,
                         padding=padding, kernel_initializer=conv_init))
        if pooling_1 == 'min':
        ...
        #FCC 1
        model.add(Dense(dense_neurons_1, activation=activation_dense, kernel_initializer=dense_init))
        model.add(Dropout(dropout_1))
        if regularizer == 'batch_norm':
            model.add(BatchNormalization())
        
        #FCC 2
        model.add(Dense(dense_neurons_2, activation=activation_dense, kernel_initializer=dense_init))
...
def print_params(batch_size, lr, decay, momentum, nesterov, loss, optimizer, regularizer, conv_init, activation_conv, dense_init, activation_dense,
    filter_size_1, filter_size_2, filter_size_3, filter_size_4, filter_size_5, pooling_1, pooling_2, pooling_3, pool_size_1, pool_size_2, pool_size_3):
    """
    ...
    Args:
        ...
        regularizer (str): The regularization method used to prevent overfitting.
    ...
    print('Convolutional AF : '+ activation_conv)
    print('Dense Layers Initializer : '+ dense_init)
    print('Dense Layers AF : '+ activation_dense)
    print()
