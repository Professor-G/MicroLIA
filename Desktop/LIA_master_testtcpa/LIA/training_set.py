...
def create(timestamps, min_mag=14, max_mag=21, noise=None, n_class=500):
    ...
    for k in range(1,n_class+1):
        for j in range(10000):
            time = random.choice(timestamps)
            baseline = np.random.uniform(min_mag,max_mag)
            mag, baseline, u_0, t_0, t_e, blend_ratio, f_s, f_b = simulate.microlensing(time, baseline)
            try:
                if noise is not None:
                    mag, magerr = noise_models.add_noise(mag,noise)
                if noise is None:
            ...
            quality = quality_check.test_microlensing(time, mag, magerr, baseline, u_0, t_0, t_e, blend_ratio, f_s,f_b)
            if quality is True:
