...
def create(timestamps, min_mag=14, max_mag=21, noise=None, zp=24, exptime=60, n_class=500, ml_n1=7, cv_n1=7, cv_n2=1, t0_dist=None, u0_dist=None, tE_dist=None, n_pca=10,test=True):
    ...
    pca = decomposition.PCA(n_components=n_pca, whiten=True, svd_solver='auto')
...
def create_tests(timestamps, min_mag=14, max_mag=21, noise=None, zp=24, exptime=60, n_class=500, ml_n1=7, cv_n1=7, cv_n2=1, t0_dist=None, u0_dist=None, tE_dist=None, n_pca=10,test=True):
    """Creates a training dataset using adaptive cadence.
    Simulates each class n_class times, adding errors from
    a noise model either defined using the create_noise
    function, or Gaussian by default
    ...
    cols = fits.ColDefs([col0, col1, col2, col3, col4])
    hdu = fits.BinTableHDU.from_columns(cols)
    hdu.writeto('lightcurves_test.fits',overwrite=True)

    np.savetxt('feats.txt',np.array(stats_list).astype(str),fmt='%s')
    with open(r'feats.txt', 'r') as infile, open(r'all_features_test.txt', 'w') as outfile:      
         data = infile.read()
         data = data.replace("'", "")
         data = data.replace(",", "")
         data = data.replace("[", "")
    ...
    coeffs = np.loadtxt('all_features_test.txt', dtype=str)
    pca = decomposition.PCA(n_components=n_pca, whiten=True, svd_solver='auto')
    pca.fit(coeffs[:,2:].astype(float))
    X_pca = pca.transform(coeffs[:,2:].astype(float))

    classes = ["VARIABLE"]*n_class+['CONSTANT']*n_class+["CV"]*n_class+["ML"]*n_class+["LPV"]*len(np.where(coeffs[:,0] == 'LPV')[0])
    np.savetxt('pca_features_test.txt',np.c_[classes,np.arange(1,len(classes)+1),X_pca[:,:82]],fmt='%s')

    if test == True:
        quality_check.test_classifier('all_features_test.txt', 'pca_features_test.txt')
